...
def aug_show(data, data_labels):
    aug_images, aug_labels, aug_names = [], [], []
    data_labels = data_labels[None, :]
    keypoints = list(zip(zip(data_labels[:, 0], data_labels[:, 3]),
                         zip(data_labels[:, 1], data_labels[:, 4]),
                         zip(data_labels[:, 2], data_labels[:, 5])))
    print(keypoints)
    for i in range(6):
        augment = A.Compose([
            A.RandomResizedCrop(width=data.shape[0], height=data.shape[1], scale=(0.8, 1.0), ratio=(0.75, 1.33), p=(i==0)),
            A.RandomBrightnessContrast(p=(i==1)),
            A.Rotate(limit=(-45, 45), p=(i==2), border_mode=4),
            A.InvertImg(p=(i==3)),
            A.VerticalFlip(p=(i==4)),
            A.HorizontalFlip(p=(i==5))
        ], keypoint_params=A.KeypointParams(format='xy', remove_invisible=False))

        augmented = augment(image=data, keypoints=keypoints[0])
...
def augment_data(data, data_labels, data_names, n=100):
    ...
    if balanceLRFlag:
        ...
        for i, idx in enumerate(idxs):
            ...
            cur_name = data_names[idx]
            augmented = augment(image = data[idx], keypoints = keypoints[idx])
            augmented_image = augmented['image']
            augmented_keypoints = augmented['keypoints']
...
# def plot_augs(og, augs, og_labels, augs_labels):
#     N = len(og) + len(augs)
#     for i in range(N):
#         plt.subplot(2, len(augs) + 1, i + 1)
#         plt.imshow(image, cmap='bone')
#         plt.scatter(lab[i, 0], lab_show[i, 3], c='red', s=5)  # superior patella loc
...
def main():
    ...
    n_aug = 10   # number of augmented images to create
    ...
    aug_images, aug_labels = aug_show(data[0], data_labels[0])
    plt.subplot(2, 7, 1)
    plt.imshow(data[0], cmap='bone')
    plt.scatter(data_labels[0][0], data_labels[0][3], c='red', s=5)  # superior patella loc
    plt.scatter(data_labels[0][1], data_labels[0][4], c='red', s=5)  # inferior patella loc
    plt.scatter(data_labels[0][2], data_labels[0][5], c='red', s=5)  # tibial_plateau loc
    plt.axis('off')
    plt.title('Original')
    plt.gcf().set_size_inches(12, 5)
    for i in range(len(aug_images)):
        plt.subplot(2, 7, i + 2)
        plt.imshow(aug_images[i], cmap='bone')
        plt.scatter(aug_labels[i][0], aug_labels[i][3], c='red', s=5)  # superior patella loc
        plt.scatter(aug_labels[i][1], aug_labels[i][4], c='red', s=5)  # inferior patella loc
        plt.scatter(aug_labels[i][2], aug_labels[i][5], c='red', s=5)  # tibial_plateau loc
        plt.axis('off')
        plt.title(label_dict[i])
        plt.gcf().set_size_inches(12, 5)

    aug_images, aug_labels = aug_show(data[3], data_labels[3])
    plt.subplot(2, 7, 8)
    plt.imshow(data[3], cmap='bone')
    plt.scatter(data_labels[3][0], data_labels[3][3], c='red', s=5)  # superior patella loc
    plt.scatter(data_labels[3][1], data_labels[3][4], c='red', s=5)  # inferior patella loc
    plt.scatter(data_labels[3][2], data_labels[3][5], c='red', s=5)  # tibial_plateau loc
    plt.axis('off')
    plt.gcf().set_size_inches(12, 5)
    for i in range(len(aug_images)):
    ...
    pathlabel = "/Users/jacobazoulay/Repos/CS231N_Project/data/CDI/cache/im_mean.npy"
    mean_im = np.load(pathlabel)
    show_image_no_norm(mean_im)
